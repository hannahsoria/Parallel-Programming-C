...
int N = 0;
double *data;
pthread_mutex_t mutex[10]; // global mutex initialized once
...
void *threadFunc(void *arg){
    ...
    // local array stroing the counts for each digit
    ...
    for (int i = thread_data->beginIndex; i< thread_data->endIndex; i++){
        ...
        local_counts[d]++; // increments the local count
    }

    for (int i=0; i<10; i++){
        pthread_mutex_lock(&mutex[i]); // lock the critical section (global_counts increment)
        global_counts[i] += local_counts[i]; // update the global array with the local array
        pthread_mutex_unlock(&mutex[i]); // unlock the critical section
    }
