...
typedef struct ThreadInfo {
    ...
    int counts[10]; // counts for each digit with the thread // the nested array
}ThreadInfo;
...
int main(int argc, char* argv[]) {
    ...
    for (int i=0; i< NUM_THREADS; i++){
        ...
        memset(info[i].counts, 0, sizeof(info[i].counts));
    ...
    for (i = 1; i < 10; i++) { // iterate 1 - 9 for the 9 digits
        int count = 0;
        for (int j=0; j < NUM_THREADS; j++){ // iterate through the number of threads
            count += info[j].counts[i]; // add the 
        }
